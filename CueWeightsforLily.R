...
write.csv(CueWeights, 'CueWeights_all_raw.csv')

########################################################################
###The stuff below is for the ARO poster presentation
#########################################################################
method = 'normalized'
CueWeights <- NULL
...
write.csv(CueWeights, 'CueWeights_all_normalized.csv')

young <- CueWeights[CueWeights$AgeGroup == 'younger',]
aveCW <- ddply(young, ~ConditionNumber+Dimensions, summarise, meanCW = mean(CW), n = length(CW), sd = sd(CW), se = sd/sqrt(n))
aveCW$ConditionNumber <- as.factor(aveCW$ConditionNumber)

young <- read.csv('younge_normalized.csv')
young <- subset(young, ConditionNumber %in% c(4,5,6,8))
aveCW <- ddply(young, ~ConditionNumber+Dimensions, summarise, n = length(CW), sd = sd(CW), CW = mean(CW), se = sd/sqrt(n))
aveCW$ConditionNumber <- as.factor(aveCW$ConditionNumber)

png('Young_Normalized_weights.png', height = 8, width = 10, units = 'in', res = 500)
ggplot(young, aes(ConditionNumber, CW, group = Dimensions, fill = Dimensions))+
  geom_bar(data = aveCW, stat = 'identity', position = position_dodge())+
  scale_x_discrete(name = "Conditions", labels=c('-3SNR', '0SNR', '+3SNR','Quiet'))+
  scale_y_continuous(name = 'Normalized Cue weights', limits = c(0,1))+
  scale_fill_manual(breaks = c('VOT', 'F0'), values = c('grey', 'black'))+
  scale_color_manual(breaks = c('VOT', 'F0'), values = c('grey', 'black'))+
  geom_errorbar(data = aveCW, aes(ymin = CW-se, ymax = CW+se), width = 0.3, size = 1, position = position_dodge(0.9))+
  theme_bw()+
  theme(axis.title.x = element_text(size = 20, face = 'bold'), axis.text.x = element_text(size = 15, face = 'bold'), axis.title.y = element_text(size = 20, face = 'bold'), axis.text.y = element_text(size = 15, face = 'bold'), legend.text = element_text(size = 10, face = 'bold'), legend.title = element_text(size = 10, face = 'bold'))
dev.off()

write.csv(young, 'younge_normalized.csv')
...
###some exploratory analysis, ignore for now
diff <- ddply(newCueWeights, ~SubjectNumber + ConditionNumber + AgeGroup, summarise, diff = -diff(CW))
diff_ave <- ddply(diff, ~ConditionNumber+AgeGroup, summarise, mean = mean(diff), n = length(diff), sd = sd(diff), se = sd/sqrt(n))
png('CWdiffolder.png', units = 'in', width = 10, height = 8, res = 500)
ggplot(diff_ave[diff_ave$AgeGroup == 'older',],aes(ConditionNumber, mean)) +
  geom_bar(stat = 'identity')+
  geom_errorbar(aes(ymin = mean-se, ymax = mean+se), width = 0.3, position = position_dodge())+
  scale_x_discrete(labels = c('-3SNR/65dB', '0SNR/65dB', '+3SNR/65dB', 'quiet/65dB'))+
  scale_y_continuous(limits = c(-0.5,0.5))+
  labs(size = 15,
       x = 'Signal to noise levels',
       y = 'Cue Weight difference score',
       title = 'Cue Weight difference in older group')+
  theme_bw()+
  theme(plot.title = element_text(hjust = 0.5), axis.title = element_text(face = 'bold', size = 18), axis.text.x = element_text(face = 'bold', color = 'black', size = 15), axis.text.y = element_text(face = 'bold', color = 'black', size = 15), legend.text = element_text(face = 'bold', color = 'black', size = 14), legend.title = element_text(face = 'bold', color = 'black', size = 14), title = element_text(face = 'bold', size = 20))
dev.off()
  
png('CWdiffyounger.png', units = 'in', width = 10, height = 8, res = 500)
ggplot(diff_ave[diff_ave$AgeGroup == 'younger',],aes(ConditionNumber, mean)) +
  geom_bar(stat = 'identity')+
  geom_errorbar(aes(ymin = mean-se, ymax = mean+se), width = 0.3, position = position_dodge())+
  scale_x_discrete(labels = c('-3SNR/65dB', '0SNR/65dB', '+3SNR/65dB', 'quiet/65dB'))+
  scale_y_continuous(limits = c(-0.5,0.5))+
  labs(size = 15,
       x = 'Signal to noise levels',
       y = 'Cue Weight difference score',
       title = 'Cue Weight difference in younger group')+
  theme_bw()+
  theme(plot.title = element_text(hjust = 0.5), axis.title = element_text(face = 'bold', size = 18), axis.text.x = element_text(face = 'bold', color = 'black', size = 15), axis.text.y = element_text(face = 'bold', color = 'black', size = 15), legend.text = element_text(face = 'bold', color = 'black', size = 14), legend.title = element_text(face = 'bold', color = 'black', size = 14), title = element_text(face = 'bold', size = 20))
dev.off()
